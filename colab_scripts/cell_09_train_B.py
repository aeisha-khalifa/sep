# ── CELL 9: Full Vicuna pipeline — Condition B (tweets + indicators) ──
# Summaries reused from cache. Indicators now computed at t-1 (no look-ahead).
# Estimated time: 6–10 hours

import os
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:True"

!mamba run -n sep python /content/sep/main.py \
    --condition        B \
    --price_dir        /content/drive/MyDrive/sn2/price/preprocessed/ \
    --tweet_dir        /content/drive/MyDrive/sn2/tweet/raw/ \
    --ohlcv_dir        /content/ohlcv/ \
    --cache_path       /content/drive/MyDrive/sep_training/summary_cache_25.pkl \
    --data_path        /content/drive/MyDrive/sep_training/sft_data_B.json \
    --output_path      /content/saved_models/sft_B \
    --rl_base_model    /content/saved_models/sft_B_merged \
    --reward_adapter   /content/saved_models/reward_B \
    --reward_model_name /content/saved_models/reward_B_merged \
    --output_dir       /content/saved_models/ppo_B/ \
    --sep_model_path   /content/saved_models/sep_model_B \
    --save_dir         /content/drive/MyDrive/sep_training/results_25/

print("Condition B training + evaluation complete.")
